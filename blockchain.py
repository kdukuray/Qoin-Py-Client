...
Receiver Public Key: {self.receiver_pub_key[27:-26]},
...
class BlockChain:
    def mine_block(self, miner_id):
        ...
        resp = requests.get("http://127.0.0.1:8000/prequantum/transactions/pending/")
        ...
        if verified_transaction:
            ...
            resp = requests.post("http://127.0.0.1:8000/prequantum/blocks/new/", data=payload, headers=headers)
        else:
    ...
    def verify_transaction(transaction: Transaction) -> bool:
        ...
        resp = requests.get(f"http://127.0.0.1:8000/prequantum/wallets/balance/{transaction.sender_id}/")
    ...
    def get_and_verify_current_block_chain_state(self):
        ...
        all_blocks_list = requests.get(f"http://127.0.0.1:8000/prequantum/blocks/after/{last_block_id}/")
        ...
        for block_data in all_blocks_list:
            ...
            all_block_transactions_data = requests.get(f"http://127.0.0.1:8000/prequantum/blocks/{block_obj.id}/transactions/")
    ...
    def verify_block(self, block_id):
        """Verifies Blocks"""
        all_block_transactions_data = requests.get(f"http://127.0.0.1:8000/prequantum/blocks/{block_id}/transactions/")
        all_block_transactions_data = all_block_transactions_data.json()
        all_block_transactions_are_valid = True
        serialized_transactions = []
        for trxn_data in all_block_transactions_data:
...
class Wallet:
    ...
    def get_wallet_balance(self):
        ...
        resp = requests.get(f"http://127.0.0.1:8000/prequantum/wallets/balance/{self.wallet_id}/")
    ...
    def make_transaction(self, receiver_pub_key, amount):
        ...
        requests.post("http://127.0.0.1:8000/prequantum/transactions/new/", data=payload, headers=headers)
